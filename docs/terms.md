...
このページは、ショップ共通の「M.G.R Tech Creations の販売物全般利用規約」（ver.20260805）の全文です。

!!! info "PDF 版について"
    同じ内容の PDF を [Google Drive](https://drive.google.com/drive/folders/1pLcx18DQoFpwCZh_F2Y6LUG_pOl5QvFB?usp=drive_link) で公開しています。**記載に差異がある場合は PDF 版が優先します。**

!!! note "2026年8月5日 改定"
    ショップ名の変更にともない、名義を「M.G.R雑貨店」から「M.G.R Tech Creations」へ更新しました。あわせて「許諾期間および許諾の変更等」の条項を更新しています。許諾条件（A〜W）の内容に変更はありません。旧版は Google Drive の `old/` に保管しています。
...
![許諾範囲の簡易一覧](assets/vn3-summary.png)
...
??? note "テキスト版（項目 A〜X）"
    ...
    | **W** 権利義務の譲渡等 | 許可しません |
    | **X** 特記事項 | こちらはM.G.R Tech Creationsの販売物全般の利用規約です。商品ごとに個別の利用規約が存在する場合、それらが優先されます。 |

---

## M.G.R Tech Creations の販売物全般利用規約

MAGURO（以下「権利者」といいます）は、M.G.R Tech Creations の販売物全般利用規約（以下「本利用規約」といいます）に従って、本利用規約に定義される本データの利用をユーザーに対して許諾（以下「本許諾」といいます）します。ユーザーは、本データの利用開始前に本利用規約の内容を必ず確認してください。なお、ユーザーは、方法を問わず本データの購入、ダウンロード、インストール等の利用の開始時点で、本利用規約に同意したものとみなします。なお、ユーザーが未成年者等の制限行為能力者である場合、本利用規約への同意にあたり、事前に保護者等の法定代理人の同意を得なければなりません。
