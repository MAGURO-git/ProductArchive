...
画像を出す板の位置と大きさは、生成された `StaffImageBoard` の Inspector で設定できます。画像行を1回押して選択し、同じ行をもう1回押す（2回押し）と板が頭の前方に出て、Pickupで動かせます。表示中の画像行には右端に白い目アイコンが点灯し、表示済みの行を再度押すと手元から非表示にできます（しまう）。ページを切り替えても表示中の板は残るため、複数枚を同時に出せます。板はプレイヤーから既定3m以上離れると自動でしまわれます。
...
2回インタラクトすると、当日スタッフを全員ぶん解除します。押せるのは常設スタッフで、かつ現在スタッフ ON の人だけです。スタッフ管理タブと同じ条件です。名簿に載っている常設スタッフは、自動ONの設定に関係なく解除されません。インスタンスに居ない人には届きません（[運用ガイド](operation.md#staff-detection) 参照）。
