...
画像を出す板の位置と大きさは、生成された `StaffImageBoard` の Inspector で設定できます。画像行を1回押して選択し、同じ行をもう1回押す（2回押し）と板が頭の前方に出て、Pickupで動かせます。板をつかんだ手のトリガーを引きながら前後に動かすことで、0.5倍〜2.0倍の範囲で拡大・縮小が可能です（手を離しても拡大率は保持されます）。表示中の画像行には右端に白い目アイコンが点灯し、表示済みの行を再度押すと手元から非表示にできます（しまう）。ページを切り替えても表示中の板は残るため、複数枚を同時に出せます。板はプレイヤーから既定3m以上離れると自動でしまわれます。
