...
無線との相互排他、スタッフでなくなったときの即時停止、使用中のステータス表示は常に有効で、設定項目はありません。

---

## StaffAreaZone（防音エリア）

エリアの入退室を判定し、中の会話が外へ届く距離を絞ります。1エリアにつき1つ配置します。スタッフはエリアの中に居てもインカムをそのまま使えます。

設置は「セットアップ」タブの単品ギミック **防音エリア** から行います。追加後、Scene ビューで BoxCollider を部屋の形に合わせてください。削除は対象を選んで **Tools ＞ MGR ＞ Staff Bridge System ＞ 防音エリアを削除（選択中）** から行います。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| エリア名 | string | エリア | 運営者が見分けるための名前（例: 相談ブース1）。ワールド内の来場者には表示されない |
| 照合間隔秒 | float | 2 | 位置照合の間隔。トリガーの取りこぼしからの自己回復に使う |

遮る距離は Inspector ではなく「編集」タブの **防音エリア** から設定します（既定は 3m まで全音量・4.5m で無音。[運用ガイド](operation.md#sound-area)）。エリアは1ワールドに16個までです。

!!! warning "エリア同士を重ねないでください"
    重なった場所では、先に登録されたエリアの所属が優先されます。意図した入れ子でなければ離してください。重なりがある場合は診断が知らせます。
...
常時アクティブなオブジェクトに置きます。実音量は「既定音量 × スライダー値」です。
...
### StaffTalkSettings（発話方式）

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| トグル発話を初期ONにする | bool | OFF | ON にすると、初期状態がトグル発話になる |
| キー候補コード | int[] | なし / T / G / B | 設定タブに並べる DT キーの候補（4つ） |
| DTキー初期番号 | int | 1 | 初期選択の候補番号（0 = 1番目 = なし） |
...
利用できるのは名簿掲載者かつ現在スタッフの人だけで、権限はタブの表示制御に加えて、表示時と確定時にも再チェックされます。
...
2回インタラクトすると、スイッチで付与されたスタッフを全員ぶん解除します。押せるのは**名簿に載っていて、かつ現在スタッフである人**だけです（スタッフ管理タブと同じ条件。後から付与されたスタッフは押せません）。初期スタッフ名一覧に載っている人は解除されません。インスタンスに居ない人には届きません（[運用ガイド](operation.md#staff-detection) 参照）。
...
| 権限なしテキスト | string | `<color=#AAAAAA>事前登録スタッフのみ操作できます</color>` | 操作する権限が無い人が触れた時の表示 |
