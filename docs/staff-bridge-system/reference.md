...
共有タイマーの残り時間を、ワールドに置いた板へ表示します。1つのワールドに何枚でも置けます（板どうしは互いを知らず、それぞれが同じ StaffOpsTimer を読みます）。
...
名簿へ登録するのは「スタッフだけに見せる」が ON のときだけです。全員に見せる板は、何枚置いてもスタッフ判定の枠（1ワールドで合計128個）を消費しません。
...
2回インタラクトすると、当日スタッフを全員ぶん解除します。押せるのは常設スタッフで、かつ現在スタッフ ON の人だけです（スタッフ管理タブと同じ条件。当日スタッフは押せません）。名簿に載っている常設スタッフは、自動ONの設定に関係なく解除されません。インスタンスに居ない人には届きません（[運用ガイド](operation.md#staff-detection) 参照）。
