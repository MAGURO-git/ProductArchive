...
メニューは開いた時点の位置と角度に固定されます。視点や身体を動かしても面は追従せず、設定の「視界表示の向き」は通知ログと視界ノートにだけ適用されます。
...
拡声している間だけ、スタッフ以外を含む全員へ声を届けます。インカム送信とは同時に使えず、片方の使用中はもう片方を起動できません。
...
画像とページ名は、セットアップ窓の **カスタマイズ > メニュー構成 > 画像資料編集を開く** から編集できます。対応形式は Sprite、または Unity が Texture2D として読み込める PNG / JPG / TGA / PSD などです。`画像資料に反映` を押すと、メニューを作り直さずに反映されます。

画像を出す板の位置と大きさは、生成された `StaffImageBoard` の Inspector で設定できます。画像を選んで「手元に出す」を押すと板が頭の前方に出て、Pickupで動かせます。ページを切り替えても表示中の板は残るため、複数枚を同時に出せます。板はプレイヤーから既定3m以上離れると自動でしまわれます。

### StaffImageBoard（画像板）

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 前方距離 | float | 0.75 | 「手元に出す」を押したとき、頭の前方へ出す距離（m） |
...
| メガホンVR初期方式 | int | 0 | VRショートカットの初期方式（0 = 長押し / 1 = トグル / 2 = OFF） |
