...
設置は「Prefab を個別に置く」ページの **防音エリア** から行います。追加後、Scene ビューで BoxCollider を部屋の形に合わせてください。削除は対象を選んで **Tools ＞ MGR ＞ Staff Bridge System ＞ 防音エリアを削除（選択中）** から行います。
...
遮る距離は Inspector ではなく「防音エリア」ページから設定します（既定は 3m まで全音量・4.5m で無音。[運用ガイド](operation.md#sound-area)）。エリアは1ワールドに16個までです。
...
| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 表示位置 | int | 0 | 出す位置の初期値（0 = 左下 / 1 = 右下 / 2 = 表示しない）。**各自が設定タブで上書きでき、その選択は PlayerData に保存されます** |
...
進行台本などを複数ページで表示します。すべてローカル動作で、同期はしません。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| ページ名 | string[] | 開場前 / 本編 / 終了後 | ページ切替ボタンに出す名前。**ページ本文と同じ数・同じ順**にする |
| ページ本文 | string[] | 3件 | 各ページの本文。TMP のリッチテキストが使える |
...
ページは最大6枚です。下の入力欄は各自の走り書きメモで、同期せず、ワールドを出ると消えます。
...
| 全員に送る | bool[] | ON = 全員の端末で実行（入場ゲートを開ける等）／ OFF = 押した本人の端末だけで実行 |

4つの配列は**同じ数・同じ順**で用意してください。セットアップ窓の **カスタマイズ ＞ スイッチ** ページなら、行単位で追加・削除でき、数がずれる心配もありません（イベント名は発火先から選べます）。
