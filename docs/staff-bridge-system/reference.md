...
「今誰がスタッフか」をインスタンス全体で同期管理する中核です。1シーンに1つだけ配置します。各ギミックはここに登録して、スタッフ状態の変化を受け取ります。
...
下の「開閉操作」「表示」は、セットアップ窓の **カスタマイズ > メニューの出かた** ページからも変えられます。
...
Desktop ではメニュー展開中、数字キー `1`〜`9`（テンキー対応）を押すことで左レールのタブを上から順に直接切り替えられます（ノートの自分用メモ入力欄など入力フォーカスがある間は自動でバイパスされます）。また、VR ではメニューの項目決定時や発話開始・終了時、テレポート時にコントローラーへの微小振動（触覚フィードバック）で操作を伝えます。
...
メニューは開いた時点の位置と角度に固定されます。視点や身体を動かしても面は追従せず、設定の「視界表示の向き」は通知ログとノートにだけ適用されます。
...
| 判定半径 | float | 0.20 | 手がその位置に来たと判定する半径（m。目安 0.15〜0.25）。目線の高さ 1.6m のアバターを基準にした値で、小さいアバターでは判定の位置と半径が自動で縮む |

VRジェスチャーの操作方式は、各自が設定タブでPTT（押している間送信）／トグル／OFFから選べます（初期値はPTT、PlayerDataに保存）。送信開始時と終了時には右手コントローラーへ触覚フィードバックが返ります。また、メニューを閉じていても視界HUDに受信中の発話者名（最大2名＋「他」）が表示されます（設定タブでON／OFF可能）。
...
拡声している間だけ、スタッフ以外を含む全員へ声を届けます。インカム送信とは同時に使えず、片方の使用中はもう片方を起動できません。拡声開始時と終了時には左手コントローラーへ触覚フィードバックが返ります。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 起動長押し秒 | float | 0.5 | ボタン・VRトリガーを保持してから拡声が始まるまでの秒数（誤タップ防止） |
| 最大使用秒 | float | 180 | 連続使用できる秒数。超えると自動で停止する |
...
ボタンの方式（長押し／トグル）、Desktopキー、VRの方式（長押し／トグル／OFF）は各自が[設定タブ](usage.md#settings)で選びます。初期値は [StaffTalkSettings](#talk-settings) が持ちます。ジェスチャーの位置（耳／肩／胸元）と判定半径は[インカム](#incom)側の設定を共通で使い、手はインカム＝右・メガホン＝左で固定です。

---

## StaffBroadcast（一斉メッセージ） { #broadcast }

定型文をスタッフへ一斉送信します。受信側は[通知ログ](#notice-log)・通知音・受信ログで受け取ります。サーバー時刻を見て、レイトジョイン時に古いメッセージが再表示されるのを防いでいます。他の在室スタッフが0人の場合でも、「全員宛」なら送信確認・リハーサルが可能です。
...
| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| プリセット | string[] | 15件 | 送信できる定型文。TMP のリッチテキストが使える。専用の編集ウィンドウあり（ドラッグ並び替え対応） |
| プリセットカテゴリ | string[] | 接客／CH誘導／移動／指示 | 各定型文のタブ分けカテゴリ名（定型文と同じ順序）。空欄は「その他」 |
...
| 自分宛プレフィックス | string | `<b>【あなた宛】</b> ` | 自分宛メッセージの先頭に付ける目印 |
...
| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 表示位置 | int | 0 | 出す位置の初期値（0 = 左下 / 2 = 表示しない / 3 = 左上。1 = 右下は廃止し、指定されていても左下へ丸める）。**各自が設定タブで上書きでき、その選択は PlayerData に保存されます** |
...
共有タイマー・ローカルタイマー・ストップウォッチをまとめて持ちます。共有タイマーだけが同期し、残りは自分専用です。プリセット加算ボタン（＋30秒／＋1分／＋5分／＋10分）による手早い設定が可能です。

### 連動発火

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 終了時トリガー | StaffTrigger | — | 共有タイマー終了時に自動発火する StaffTrigger（未設定なら発火なし） |
| 終了時トリガー番号 | int | 0 | 終了時トリガーで発火するトリガー番号（0始まり） |
...
| 到達通知音 | AudioSource | 生成時に配線 | 共有・ローカルタイマーの到達時に鳴らす短い音。個人設定の通知音量に従う |

共有タイマーの操作ができるのはスタッフだけです。時間切れは[通知ログ](#notice-log)に「共有タイマー ◯分が経過しました」と流れます。ローカルタイマーは「ローカルタイマー」と出るので、ほぼ同時に鳴っても見分けられます。

---

## StaffTimerBoard（共有タイマーボード） { #timer-board }

共有タイマーの残り時間を、ワールドに置いた板へ表示します。1つのワールドに何枚でも置けます。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| タイマー | StaffOpsTimer | — | **必須**。残り時間の出どころ。SBSメニューのタイマーを指します |
| registry | StaffRegistry | — | 「スタッフだけに見せる」を ON にするときだけ必須 |
| 見出し表示 | TMP_Text | — | **必須**。見出しを出す TMP |
| 残り時間表示 | TMP_Text | — | **必須**。残り時間を出す TMP |
| 見出し | string | 共有タイマー　残り時間 | 板に出す見出し。「開場まで」「次の回まで」など用途に合わせて変えられます |
| 未設定テキスト | string | `--:--` | 共有タイマーに何も設定されていないときの表示 |
| スタッフだけに見せる | bool | OFF | ON にするとスタッフ以外には板ごと見えなくなります |

全員に見せる板は、何枚置いてもスタッフ判定の枠（1ワールドで合計128個）を消費しません。枠を使うのは「スタッフだけに見せる」を ON にした板だけです。

---

## StaffLogBoard（ログボード） { #log-board }

SBS に起きたことを、ワールドに置いた板へ時刻順に流します。新しいものが下に足されていきます。板からSBSを操作することはできません。見るための板です。1つのワールドに何枚でも置けます。

板の上のボタンで種別を絞り込めます（すべて／スイッチ／メッセージ／タイマー／スタッフ／入退室）。
板の右端のボタンで過去へさかのぼれます（▲＝古い方へ／▼＝新しい方へ／最新へ）。送りは1画面ぶん（ログ行数）ずつです。
どちらも押した人の画面にだけ効きます。

さかのぼっている間は、新しい行が来ても表示は動きません。溜まった件数は「最新へ」に出ます。絞り込みを押すと最新に戻ります。

載せるのは**その場に居る全員が同じものを見られる出来事だけ**です。

| 種別 | 内容 |
|---|---|
| スイッチ | 誰が・どのスイッチを押し、**どの発火先のどのイベントへ送ったか**。入室前の分も同期履歴からさかのぼって出ます |
| メッセージ | 誰が・どの宛先へ送ったか。**板が動き出してからの分だけ**です |
| タイマー | 共有タイマーの開始と終了。誰が操作したかは本製品が記録していないため出ません |
| スタッフ | スタッフ権限の増減 |
| 入退室 | 誰が入ってきて、誰が出ていったか |

**テレポート・インカム・メガホンは載りません。** どれも同期しておらず、自分の操作しか読めないためです。板に出すと人によって中身が違うものになります。

例外として、押した人にしか出ない警告2つ（発火先が未設定・連打止めで弾いた）は「（あなたにだけ）」と付けて載せます。配線ミスに気づく手がかりだからです。

**個人宛・グループ宛のメッセージは宛先だけを出し、本文は伏せます。** 板は宛先でない人も見るためです。全員宛は本文まで出ます。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| スイッチ | StaffTrigger | — | 入れた機能だけログに出ます。SBSメニューの StaffTrigger を指します |
| メッセージ | StaffBroadcast | — | 同上 |
| タイマー | StaffOpsTimer | — | 同上 |
| registry | StaffRegistry | — | 権限の増減と、下端の状態バーに使います |
| 見出し表示 | TMP_Text | — | **必須**。見出しを出す TMP |
| ログ表示 | TMP_Text | — | **必須**。ログを出す TMP |
| 状態表示 | TMP_Text | — | **必須**。下端の状態バーを出す TMP |
| 見出し | string | SBS ログ | 板に出す見出し |
| ログ行数 | int | 14 | 板に出す行数。板の高さに合わせてあります。増やすと下からはみ出します |
| ログ保持数 | int | 140 | 内部に持っておく行数。さかのぼれる量です。同期しないので増やしても通信は増えません |
| 位置表示 / 古い方ラベル / 新しい方ラベル / 最新へラベル | TMP_Text | — | 右端の送りの文字。板を置いたときに自動で入ります |
| スタッフだけに見せる | bool | OFF | ON にするとスタッフ以外には板ごと見えなくなります |

板の下端には、いまの状態が1行で常に出ます（スイッチの配線の欠け・スタッフ人数・自分がスタッフか・共有タイマー）。

分かるのは「送った」ところまでです。押されたイベントを受けたギミックが何をしたかは本製品からは分かりません。

この板は押すボタンを持つため、VRChat のカメラに映ります。
...
通常は Inspector ではなく、セットアップ窓の **カスタマイズ > メニュー構成 > ノート編集を開く**（または `Tools > MGR > Staff Bridge System > ノート編集`）から編集してください。ページの追加・≡によるドラッグ並べ替え・本文の編集がまとめて行え、2つの配列の数が食い違う心配もありません。
...
## StaffImagePanel（画像資料） { #image }

ワールド制作者が用意した画像を、SBSメニューで確認して本人の手元へ出します。表示状態はローカルのみで、他の人へは同期されません。

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 画像 | Sprite[] | 8枠 | メニューに表示する画像。最大16枚。空の枠は押せません。画像の容量はワールド容量に加算されます |
| ページ名 | string[] | 空 | ページボタンに出す名前。1件8文字まで。空欄は「資料1」のような既定名になります |

画像とページ名は、セットアップ窓の **カスタマイズ > メニュー構成 > 画像資料編集を開く** から編集できます。対応形式は Sprite、または Unity が Texture2D として読み込める PNG / JPG / TGA / PSD などです。編集窓では「≡」ドラッグによる並べ替えに対応しており、`画像資料に反映` を押すとメニューを作り直さずに反映されます。

画像を出す板の位置と大きさは、生成された `StaffImageBoard` の Inspector で設定できます。画像行を1回押して選択し、同じ行をもう1回押す（2回押し）と板が頭の前方に出て、Pickupで動かせます。板の下の「大きさ」スライダーで大きさを変えられます（Desktop・VR共通）。VRでは、板をつかんだままトリガーを引き、右スティックを上へ倒すと拡大、下へ倒すと縮小することもできます（既定で0.5倍〜2.0倍。手を離しても、しまって出し直しても大きさは保持されます）。表示中の画像行には右端に白い目アイコンが点灯し、表示済みの行を再度押すと手元から非表示にできます（しまう）。ページを切り替えても表示中の板は残るため、複数枚を同時に出せます。板はプレイヤーから既定3m以上離れると自動でしまわれます。

### StaffImageBoard（画像板）

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 前方距離 | float | 0.75 | 行を2回押して出したとき、頭の前方へ出す距離（m） |
| 下方オフセット | float | 0.18 | 頭の位置から下へずらす距離（m）。Desktopでは板の下のスライダーが画面に入るよう、この値より0.08m上に出す |
| 最大の高さ | float | 560 | 板の高さの上限（px） |
| 最大の幅 | float | 1000 | 板の幅の上限（px） |
| 自動でしまう距離 | float | 3 | この距離（m）以上離れると自動でしまう。0以下で無効 |
| 右スティックで大きさを変える | bool | ON | 板を持ってトリガーを引いている間、右スティックの上下で大きさを変えられるようにする |
| 大きさを変える速さ | float | 1.2 | 大きさが変わる速さ。大きいほど速い（目安 0.8〜2.0） |
| 最小倍率 | float | 0.5 | 縮められる下限。生成時の大きさを1とした倍率。スライダーの左端 |
| 最大倍率 | float | 2 | 広げられる上限。生成時の大きさを1とした倍率。スライダーの右端 |

---

## StaffTrigger（スイッチ） { #trigger }

SBSメニューのボタンから、ワールド内の任意のギミックへイベントを1つ送ります。
...
3つの配列は同じ数・同じ順で用意してください。セットアップ窓の **カスタマイズ > スイッチ** ページなら、行単位で追加・削除でき、数がずれる心配もありません（イベント名は発火先から選べます）。
...
イベントを送るのは押した本人のクライアントだけです。全員の画面に反映するかどうかは送り先のギミックが決めます。
...
| メガホンVR初期方式 | int | 0 | メガホンVRショートカットの初期方式（0 = 長押し / 1 = トグル / 2 = OFF） |
...
登録地点への移動と、在室スタッフの元への移動を担当します。移動はローカルのみのため同期は不要です。VR では1回目の選択時（0.03秒）と移動確定時（0.10秒）に右手コントローラーへ触覚フィードバックが返ります。

### 行き先

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| 行き先一覧 | Transform[] | 空 | テレポート先（位置と向き）。専用の登録ウィンドウあり（ドラッグ並び替え対応）。**上限16件** |
| 行き先名一覧 | string[] | 空 | 各ボタンの表示名（行き先一覧と同じ順序）。未入力なら Transform 名を使う |
| 未選択テキスト | string | 行き先を選択してください | 行き先未選択時の表示 |

地点のお気に入り（★）は PlayerData に保存され、再入場後も保持されます（スタッフのお気に入りは退室するとリセットされます）。

### 着地位置

| 項目 | 型 | 既定 | 内容 |
|---|---|---|---|
| ばらつかせる | bool | ON | 登録地点へ飛ぶとき、着地位置を水平方向へランダムにずらす（重なり防止）。在室スタッフの元へ飛ぶときと「戻る」には適用されない |
| ばらつき最小半径 | float | 0.3 | 行き先から最低どれだけ離れて降りるか。0 にすると行き先ちょうどにも降りる |
| ばらつき最大半径 | float | 0.8 | 行き先から最大どれだけ離れて降りるか。広いほど重なりにくいが、狭い場所では壁の外や物の中へ出やすい |
| スタッフ背後距離 | float | 0.1 | スタッフの元へ飛ぶとき、相手の背後何 m に降りるか。離すほど、壁際のスタッフへ飛んだときに壁の外へ出やすい |

この4項目は、セットアップ窓の **カスタマイズ > 既定値** ページ（■ テレポート）からも変えられます。
...
これらのコンポーネントは `registry`（StaffRegistry）の割り当てが必須です。単品ボタンや右クリックメニューから設置した場合は自動で割り当てられます。
