...
SBS に起きたことを、ワールドに置いた板へ時刻順に流します。新しいものが下に足されていきます。板からは何も操作できません。1つのワールドに何枚でも置けます。

板の上のボタンで種別を絞り込めます（すべて／スイッチ／メッセージ／タイマー／スタッフ／入退室）。切り替えは押した人の画面にだけ効きます。
...
| ログ保持数 | int | 40 | 内部に持っておく行数。絞り込みで拾えるよう、出す行数より多く持ちます |
| スタッフだけに見せる | bool | OFF | ON にするとスタッフ以外には板ごと見えなくなります |
