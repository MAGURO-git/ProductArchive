...
画像を出す板の位置と大きさは、生成された `StaffImageBoard` の Inspector で設定できます。画像行を1回押して選択し、同じ行をもう1回押す（2回押し）と板が頭の前方に出て、Pickupで動かせます。板をつかんだままトリガーを引き、右スティックを上へ倒すと拡大、下へ倒すと縮小します（既定で0.5倍〜2.0倍。手を離しても大きさは保持されます）。表示中の画像行には右端に白い目アイコンが点灯し、表示済みの行を再度押すと手元から非表示にできます（しまう）。ページを切り替えても表示中の板は残るため、複数枚を同時に出せます。板はプレイヤーから既定3m以上離れると自動でしまわれます。
...
| 下方オフセット | float | 0.18 | 頭の位置から下へずらす距離（m） |
...
| 最小倍率 | float | 0.5 | 縮められる下限。生成時の大きさを1とした倍率 |
| 最大倍率 | float | 2 | 広げられる上限。生成時の大きさを1とした倍率 |
