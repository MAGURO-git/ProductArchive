...
| ノート | 全員に見える共有メモと、自分だけの走り書きメモ |
| スイッチ | ワールド内のギミックへイベントを送る |
| 設定 | 音量・UI サイズ・発話方式・通知ログの個人設定 |
| スタッフ管理 | 当日スタッフの追加と削除（常設スタッフのみ） |

![SBSメニュー（テレポートタブ）](images/menu-teleport.png)

「スタッフ管理」タブが薄いのは、常設スタッフ以外には開けないためです。

使わないタブは非表示にできます。並び順も変えられます（[メニュー構成](howto.md#menu-config)）。タブはメニュー左側に縦に並びます。
...
- **お気に入り**: 各行の ★ を押すと一覧の上部に固定されます。インスタンス内でのみ有効で、退室するとリセットされます（スタッフのお気に入りは32人まで）。
...
![SBSメニュー（インカムタブ）](images/menu-incom.png)

**スタッフ限定モード**（既定 ON）では、送信中の声が届くのは同じチャンネルのメンバーだけです。スタッフ以外や別チャンネルのスタッフには届きません。この設定はワールド制作者が決めるもので、インスタンス全員に適用されます。
...
発話をやめてから実際に送信が切れるまで、わずかな待ち時間（既定 0.3 秒）があります。語尾が切れるのを防ぐためです。また、押しっぱなしの事故を防ぐために既定 120 秒で強制的に送信が切れます。
...
![SBSメニュー（参加メンバー一覧）](images/menu-incom-members.png)

## メガホン { #megaphone }

拡声している間だけ、距離に関係なく**インスタンス内の全員（スタッフ以外を含む）**へ声が届きます。始め方は3通りあります。

- インカムパネルのメガホンボタンを**長押し**（既定 0.5 秒）
- 設定タブで割り当てた **Desktop キー**
- VR では**左手**を耳（設定した位置）へ近づけてトリガーを保持（既定 0.5 秒）

開場・終了の案内や誘導など、スタッフ以外への呼びかけに使います。誤操作を防ぐため、起動と停止は次のように動作します。

- ボタンからの起動は長押し。誤タップでは始まりません
- 長押し方式は、ボタンやトリガーを離すと止まります
- VR ショートカットは設定タブの「メガホンVR」で長押し／トグル／OFFを選べます。トグルも起動には保持が必要で、停止は次のトリガー押下で即時です
- 「メガホンVR」の方式を変更すると、使用中のメガホンも止まります。OFFにしてもメニューのボタンとDesktopキーは使えます
- トグル方式はメニューを閉じても続きます。使い終わったらもう一度押して止めてください
...
![SBSメニュー（メッセージタブ）](images/menu-message.png)
...
![SBSメニュー（所属グループの選択）](images/menu-message-groups.png)

- 班のボタンは押すたびに入る／抜けるが切り替わります。**いくつでも掛け持ちできます**
- 「どのグループにも所属しない」を選ぶと、どのグループ宛も受け取らなくなります
- 選んだ所属は再入場後も引き継がれます
- 宛先グループのチップには現在の人数が出ます。誰も居ないグループは選べません
...
![SBSメニュー（受信ログ）](images/menu-messagelog.png)
...
出す位置は各自が[設定タブ](#settings)から左下・左上・表示しないの3つから選べます。メニューを開いていなくても表示されます。
...
![SBSメニュー（タイマー・ログタブ）](images/menu-timer.png)

**共有タイマー**は、スタッフ全員に同じ残り時間が見えます。＋30秒／＋1分／＋5分で時間を積み、開始すると減っていきます。誤操作を防ぐため、**開始と取消は2回押し**です（1回目でボタンの文言が変わり、既定 5 秒以内にもう一度押すと確定します）。0 になると通知ログに「共有タイマー ◯分が経過しました」と出ます。

**ローカルタイマー**は自分だけのタイマーです。他のスタッフには見えず、確認の2回押しもありません。**ストップウォッチ**も自分専用で、経過時間を計ります。

共有・ローカルタイマーが0になると、短い通知音が1回鳴ります。音量は設定タブの「通知音」で調整でき、0にすると無音になります。通知ログを「表示しない」にしても音は鳴ります。
...
進行台本やチェックリストなどを、ワールド内で読むためのタブです。

![SBSメニュー（ノートタブ）](images/menu-note.png)

- **ページ**: 上のボタンでページを切り替えます（既定は「開場前」「本編」「終了後」の3ページ）。内容とページ名は Unity 側で自由に書き換えられ、**全スタッフに同じものが見えます**
- **メモ欄**: 右半分は自分だけの走り書きです。他のスタッフには見えません。打った内容は再入場後も引き継がれます。入力は12行までです。上限を超える入力や貼り付けは取り消され、操作音が鳴ります
- **視界ノート**: 各欄の下の「共有メモを表示」「自分用メモを表示」で、表示中の共有ページと自分のメモを、それぞれ視界の右下に出したままにできます（両方同時も可）。メニューを閉じたままでも読めます。文字の大きさは[設定タブ](#settings)で小・中・大から選べ、表示の選択・行数・大きさは再入場後も引き継がれます
- **視界に使う行数**: 切替ボタンの隣の −/＋ は、視界の板に使う行数です（共有 1〜15・メモ 1〜12）。板2枚で使える縦幅には上限があり（VR は広め・Desktop は画面に収まる範囲）、入らなくなる ＋ は薄くなって押せません。下端のバーが今の取り分（左から共有・メモ・残り）を示します。メモの板は書いた行数までしか場所を取りません

## スイッチ { #switch }

手元から、ワールド内のギミックへイベントを送るタブです。開場ゲートを開ける、演出を出すといった操作を、その場所まで行かずに実行できます。

![SBSメニュー（スイッチタブ）](images/menu-switch.png)
...
- 「実行履歴」ボタンで、誰がいつどのスイッチを押したかを直近64件までさかのぼれます。履歴も全スタッフ共通で、あとから入った人にもそれまでの分が見えます

!!! note "押した結果は送り先のギミック次第です"
    このタブが保証するのは「イベントを1回送る」ところまでです。送り先が全員の画面で同じ状態になるかどうかは、送り先のギミックの作りによります。同梱の `StaffObjectToggle` を使う場合は、**同期する**の ON / OFF でグローバル・ローカルを選べます。[サンプルワールド](https://vrchat.com/home/world/wrld_01cbbb48-44ce-4761-9baa-6f4ef87fbbfd/info)の STEP 8 には、その両方（ビーコン柱＝グローバル／空の色＝ローカル）を置いてあります。

## 設定 { #settings }

設定タブの変更は、自分にだけ適用されます。設定は VRChat の PlayerData に保存され、同じワールドへ入り直したときに復元されます。

![SBSメニュー（設定タブ）](images/menu-settings.png)

| 項目 | 内容 |
|---|---|
| 操作音 | UI 操作音・インカムの送信音・メッセージ送信音の音量 |
| 通知音 | インカムの受信ビープ・メッセージ通知音・タイマー到達音の音量 |
...
| メガホンVR | 左手のVRショートカットの方式（長押し／トグル／OFF） |
| 通知ログ | [通知ログ](#notice-log)を出す位置（左下／左上／表示しない） |
| 視界ノートの文字 | [ノート](#note)を視界に出したときの文字の大きさ（小／中／大） |

「ボタン」と「Desktop キー」は、**インカムとメガホンを行に、設定を列にした表**になっています。インカムをトグルにしつつメガホンは長押しのまま、といった組み合わせが選べます。メガホンの Desktop キーの既定は割り当てなしです。使う人が自分で決めます。同じキーは両方に割り当てられず、あとから選んだ側が優先されてもう片方が OFF になります。

いずれも「設定を初期化」ボタンで既定値に戻せます。操作音だけを小さくし、通知音は聞こえるようにしておくこともできます。

!!! warning "トグル発話について"
    トグルにすると、**メニューを閉じても送信が続きます**。送信中は視界のステータス表示で確認できます。最大送信秒を超えると自動で停止します。インカムのVRジェスチャーは押している間だけです。メガホンのVRジェスチャーは「メガホンVR」で選んだ方式に従います。
...
このタブが使えるのは、**常設スタッフで、かつ現在スタッフ ON の人**だけです。他のスタッフにはタブが薄く表示され、開けません。

![SBSメニュー（スタッフ管理タブ）](images/menu-grant.png)
...
- 各一覧の表示枠は既定32人です。一覧に収まらない人数は「他 N 人」と表示されます
...
| 種類 | 挙動 |
|---|---|
| スタッフ専用スイッチ | スタッフだけが操作できる。スタッフ以外が押すと「スタッフ専用です」と表示される |
| スタッフゲート | スタッフ以外だけが通れない壁 |
