...
- VR では**左手**を耳（設定した位置）へ近づけてトリガー

スタッフ以外へ呼びかける場面（開場・終了案内、誘導）を想定した機能です。インカムは「スタッフ以外に聞かせない」ための仕組みです。メガホンはその唯一の例外にあたるため、事故が起きにくいように次の制約があります。
...
**共有タイマー**は、インスタンスの全員に同じ残り時間が見えます。＋30秒／＋1分／＋5分で時間を積み、開始すると減っていきます。誤操作を防ぐため、**開始と取消は2回押し**です（1回目でボタンの文言が変わり、既定 5 秒以内にもう一度押すと確定します）。0 になると通知ログに「共有タイマー ◯分が経過しました」と出ます。
...
- **メモ欄**: 右半分は自分だけの走り書きです。他のスタッフには見えません。打った内容は再入場後も引き継がれます。書けるのは**欄に見えている12行まで**で、入らない打鍵・貼り付けは直前の状態に戻り、操作音で知らされます
