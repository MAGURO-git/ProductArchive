...
!!! warning "スタッフ限定モードを OFF にした場合"
    無線としては同じチャンネルにしか届きませんが、**近くにいる人には通常の声（距離減衰あり）として聞こえます**。運営連絡をスタッフ以外に聞かれたくない場合は、既定の ON のまま使ってください。
...
VR ジェスチャーの位置は耳・肩・胸元から選べます。判定範囲の半径も調整可能です。使う手は無線＝右手・メガホン＝左手で固定です（同じ動作で取り合いにならないように分けてあります）。
...
| ボタン | 無線・メガホンそれぞれの発話ボタンの方式（長押し／トグル） |
| Desktop キー | 無線・メガホンそれぞれで、ボタンの代わりに使うキー |
| 通知ログ | [通知ログ](#notice-log)を出す位置（左下／表示しない） |
| 視界ノートの文字 | [ノート](#note)を視界に出したときの文字の大きさ（小／中／大） |

「ボタン」と「Desktop キー」は、**無線とメガホンを行に、設定を列にした表**になっています。無線をトグルにしつつメガホンは長押しのまま、といった組み合わせが選べます。メガホンの Desktop キーの既定は割り当てなしです（キーを知らない人の誤爆を避けるため、使う人が自分で決めます）。同じキーを両方に割り当てることはできず、あとから選んだ側が優先されてもう片方が OFF になります。
