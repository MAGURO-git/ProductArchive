...
**スタッフ限定モード**（既定 ON）では、送信中の声が届くのは同じチャンネルのメンバーだけです。来場者や別チャンネルのスタッフには届きません。この設定はワールドに焼き込まれ、インスタンス全員に一律で適用されます。
...
- 中に居る人同士は、VRChat の通常の声のまま会話できます（声の設定を書き換えません）
...
出す位置は各自が[設定タブ](#settings)から左下・右下・表示しないの3つから選べます。メニューを開いていなくても表示されます。
...
- **メモ欄**: 下の入力欄は自分だけの走り書きです。他のスタッフには見えず、ワールドを出ると消えます。右上の「コピー」で今開いているページの本文をメモ欄に取り込め（そこから書き足せます）、「クリア」でメモを消せます
...
!!! note "押した結果は送り先のギミック次第です"
    このタブが保証するのは「イベントを1回送る」ところまでです。送り先が全員の画面で同じ状態になるかどうかは、送り先のギミックの作りによります。
...
| 通知ログ | [通知ログ](#notice-log)を出す位置（左下／右下／表示しない） |
