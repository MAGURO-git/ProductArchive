...
| カスタマイズ | メニュー構成 | 手元メニューのタブの並べ替え・ON / OFF、テレポート先と定型メッセージの編集 |
| カスタマイズ | テーマ・フォント | 配色とフォント |
| カスタマイズ | インカム | チャンネル数・チャンネル名・一時停止 |
| カスタマイズ | 防音エリア | エリア名と遮る距離 |
| カスタマイズ | スタッフ名簿 | 常設スタッフの登録、`.asset` の書き出し／読み込み |
| カスタマイズ | 既定値 | メガホン・無線・通知音などの初期値 |
...
一覧ボード・専用スイッチ・ゲート・防音エリアはここに出てきません。置き場所を選ぶものなので、「Prefab を個別に置く」ページ（後述）で狙った場所に置くほうが早く済むためです。
...
| 防音エリア | 中の会話が外に届かなくなるエリア |

防音エリアは音声を扱うため、先に「セットアップする」（インカムあり）を1回実行してから追加してください。追加後、Scene ビューで BoxCollider を部屋の形に合わせ、名前と遮る距離を「防音エリア」ページで決めます（[防音エリア](usage.md#area-voice)）。

頭上マーカーは単品ボタンがありません。使う場合は `Prefabs/StaffBridgeSystem_OverheadMarker.prefab` をシーンへ置いてください（名簿への接続は「診断・修復」ページで行えます）。
