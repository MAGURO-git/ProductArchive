...
| 編集 | よく使う設定をまとめた場所。テレポート先・定型メッセージ・テーマ・チャンネル名・防音エリア・各種既定値・名簿 |
...
一覧ボード・専用スイッチ・ゲート・防音エリアはここに出てきません。置き場所を選ぶものなので、単品ボタン（後述）で狙った場所に置くほうが早く済むためです。
...
防音エリアは音声を扱うため、先に「セットアップする」（インカムあり）を1回実行してから追加してください。追加後、Scene ビューで BoxCollider を部屋の形に合わせ、名前と遮る距離を「編集」タブで決めます（[防音エリア](usage.md#area-voice)）。
