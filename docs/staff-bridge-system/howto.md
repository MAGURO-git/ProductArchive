...
| ページ | 内容 |
|---|---|
| メニュー構成 | タブの並べ替えと ON / OFF、メニューの作り直し、テレポート先の編集、プリセットメッセージ編集、ノート編集 |
| メニューの出かた | 開く長押し秒・Desktop開閉キー・スティックのしきい値、表示スケール／距離／下オフセット、自動で閉じる距離、最初に開くタブ |
| テーマ・フォント | 配色テーマの選択、カスタム色、フォント、生成済みメニューへの再着色 |
| インカム | CH数（2〜12）の変更、各チャンネルの表示名、スタッフ限定モード・送信解除の待ち秒・ジェスチャー発話、インカムの一時停止 |
| 防音エリア | エリア名、外へ届く距離、BoxCollider との連動 |
| スタッフ名簿 | 最初からスタッフにしておく人の編集、`.asset` への書き出し／読み込み |
| 既定値 | メガホンの長押し秒・最大使用秒、無線の最大送信秒、通知音の初期音量、通知ログ・共有タイマー・入退室ログの既定値、DTキー発話の候補 |
...
| スイッチの発火先を決めたい | スイッチページ ＞ **＋ 行を追加** → ラベル／発火先／イベント名／全員に送る を入れる（発火先だけはヒエラルキーからドラッグ、または右の◎から選ぶ） |
...
| 来場者に無線を聞かせたくない | インカムページ ＞ **スタッフ限定モード** を ON のまま使う（既定 ON・ワールドに焼き込み） |
| 語尾が切れる | インカムページ ＞ **送信解除の待ち秒**（既定 0.3。目安 0.2〜0.4） |
| 耳元ジェスチャーの位置・手を変えたい | インカムページ ＞ **ジェスチャーの位置**（耳／肩／胸元）と **発話する手** |
