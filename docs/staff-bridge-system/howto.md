...
| 画像資料を登録・差し替えたい | メニュー構成ページ > **画像資料編集を開く**。画像とページ名を最大16枚まで編集し、**画像資料に反映**を押す。メニューの作り直しは不要 |
...
| ジェスチャーが反応しにくい／誤爆する | インカムページ > **判定半径**（既定 0.20m。目安 0.15〜0.25）。目線の高さ 1.6m のアバターを基準にした値で、小さいアバターでは自動で縮みます |
...
| やりたいこと | 触る場所 |
|---|---|
| VRメガホンの誤操作を避けたい | ワールド内の設定タブ > **メガホンVR** を長押し／トグル／OFFから選択 |
