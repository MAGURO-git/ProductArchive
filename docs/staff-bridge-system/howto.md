...
よく使う設定は、セットアップ窓の「カスタマイズ」グループにまとめてあります。まずはそこを見てください。Inspector を開く必要があるのは、めったに触らない項目だけです。

表記の見かた：

- **セットアップ窓** = メニューバーの `Tools > MGR > Staff Bridge System > セットアップ`
- **SBSメニュー** = ワールド内でスタッフが開くメニュー
...
| ページ | 内容 |
|---|---|
| メニュー構成 | タブの並べ替えと ON / OFF、メニューの作り直し、テレポート先の編集、定型文編集、ノート編集、画像資料編集（各編集窓はドラッグ並び替え対応） |
| メニューの出かた | 開く長押し秒・Desktop開閉キー・スティックのしきい値、表示スケール／距離／下オフセット、自動で閉じる距離、最初に開くタブ |
| テーマ・フォント | 配色テーマの選択、カスタム色、フォント、生成済みメニューへの再着色 |
| インカム | チャンネル数（2〜16）の変更、各チャンネルの表示名、スタッフ限定モード・送信解除の待ち秒・ジェスチャー発話、インカムの一時停止 |
| スイッチ | ワールドのギミックを動かすスイッチの行（ラベル・発火先・イベント名）の編集 |
| スタッフ名簿 | 最初からスタッフにしておく人の編集、`.asset` への書き出し／読み込み |
| 既定値 | メガホンの長押し秒・最大使用秒、インカムの最大送信秒、通知音の初期音量、通知ログ・共有タイマー・入退室ログの既定値、テレポートの着地位置、Desktopキー発話の候補 |

!!! info "いつシーンに反映されるか"
    各ページの入力は下書きです。上部に固定されているバーの 「変更を反映」 を押すまで、シーンは変わりません。下書きが入っている欄には `*` が付き、バーに件数が出ます。

    反映は `Ctrl+Z` 一回でまとめて取り消せます。反映したあと、シーンの保存（`Ctrl+S`）とワールドの再アップロードは別途必要です。

    作り直しを伴うもの（チャンネル数の変更、テーマの再着色、テレポート先と定型文の編集ウィンドウ）は下書きにできません。それぞれのボタンで即時に実行されます。
...
| 作り直すと設定が消えないか知りたい | テレポート先・チャンネル名・定型文・ノート・画像資料・スイッチの設定・既定値・メニューの位置は引き継がれます。メニュー自体は新しいオブジェクトになるため、外部から参照している場合は貼り直しが必要です |
| 画像資料を登録・差し替えたい | メニュー構成ページ > **画像資料編集を開く**。画像とページ名を最大16枚まで編集（≡でドラッグ並べ替え可）し、**画像資料に反映**を押す。メニューの作り直しは不要 |
| ノートの内容を書き換えたい | メニュー構成ページ > **ノート編集を開く**（ページの追加・≡でドラッグ並べ替え・本文の編集。最大6ページ） |
| スイッチの発火先を決めたい | スイッチページ > **＋ 行を追加** → ラベル／発火先／イベント名 を入れる（発火先だけはHierarchyからドラッグ、または右の◎から選ぶ） |
...
| 共有タイマー終了時にギミックを動かしたい | Inspector：`StaffOpsTimer` の **終了時トリガー** に `StaffTrigger` を指定し、**終了時トリガー番号** を設定 |
...
| スタッフ以外が勝手にスタッフになるのを防ぎたい | セットアップで置かれた切替スイッチを削除するか、スタッフ以外が入れない場所へ移し、管理タブからの付与に寄せる |
...
| チャンネルを増やす／減らす | インカムページ > **チャンネル数**（2〜16）を決めてボタンを押す |
| 送信しっぱなしを防ぎたい | 既定値ページ > **インカムの最大送信秒**（既定 120） |
| Desktop のキー発話の候補を変えたい | 既定値ページ > Desktopキー発話の候補（枠1は OFF 固定）。インカムとメガホンで共通の候補です |
...
| ジェスチャーが反応しにくい／誤操作を防ぎたい | インカムページ > **判定半径**（既定 0.20m。目安 0.15〜0.25）。目線の高さ 1.6m のアバターを基準にした値で、小さいアバターでは自動で縮みます。各自が設定タブでOFFにすることもできます |
| 押しっぱなしをやめてトグルやOFFにしたい | 各自がメニューの[設定タブ](usage.md#settings)で切り替え（ボタンは長押し／トグル、VRの耳元操作はPTT／トグル／OFF）。初期値は Inspector の `StaffTalkSettings` |
| 受信時に誰が話しているかを視界で見たい | 各自がメニューの[設定タブ](usage.md#settings)で **受信時の発話者名** を ON（既定 ON） |
...
| やりたいこと | 触る場所 |
|---|---|
| VRメガホンの誤操作を避けたい | ワールド内の設定タブ > **VRの耳元操作（メガホン）** を長押し／トグル／OFFから選択 |
...
インカムもメガホンも、プレイヤーの声の設定を書き換えて動きます。同じ設定を操作する別のギミック（防音室・ボイスゾーン・音量調整・拡声など）とは同時には使えません。どちらを使う回かに応じて、本製品側を止められます（[運用ガイド](operation.md#incom-power)）。
...
| やりたいこと | 触る場所 |
|---|---|
| 定型文をイベント用に書き換えたい | メニュー構成ページ > 定型文編集（≡でドラッグ並べ替え可） → [運用ガイド](operation.md#broadcast-presets) |
| よく使う文をまとめたい | 同じ画面でカテゴリ名を揃える（カテゴリごとにタブが分かれる） |
| 緊急の文を目立たせたい | 定型文の本文に TMP のリッチテキスト（`<color=...>` など）を使う |
...
| 宛先グループの名前を「受付」などにしたい | Inspector：`StaffMsgGroups` の **グループ名**（既定 A班〜H班）。名前だけなら再生成は不要。数を変えるときはメニュー構成ページからの再構築が要ります |
...
| やりたいこと | 触る場所 |
|---|---|
| 行き先を登録・並べ替えたい | メニュー構成ページ > テレポート先の編集（**上限16件**・≡でドラッグ並べ替え可） |
| ボタンの表示名を変えたい | 同じ画面で名前を入力（未入力なら Transform 名） |
| 着地が重なるのを防ぎたい | 既定値ページ > **着地点をばらつかせる**（既定 ON）。ずらす幅は **ばらつきの最小／最大半径**（既定 0.3／0.8m）。登録地点へ飛ぶときだけ効きます |
| スタッフの元へ飛ぶ距離を変えたい | 既定値ページ > **スタッフの背後距離**（既定 0.1m）。広い会場なら 0.5〜1.0 でも構いませんが、離すほど壁際のスタッフへ飛んだときに壁の外へ出やすくなります |
...
| タイマー到達音を消したい | ワールド内の設定タブ > **通知音** を0にする（ほかの通知音も無音になります） |
