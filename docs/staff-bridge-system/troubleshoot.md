...
エリアが遮るのは距離です。**「外へ届かなくなる距離」より内側に立たれた場合は聞こえます。**「防音エリア」ページで距離を小さくするか、部屋どうしをその距離より離して配置してください（[防音エリア](usage.md#area-voice)）。
...
- **イベント名が違う**可能性があります。スペルミスや大文字小文字の違いはコンパイルエラーにならず、無音で失敗します。`StaffTrigger` の **イベント名** を送り先の実装と突き合わせてください
- **全員に送る**が OFF だと、押した本人の端末でしか実行されません。他の人にも反映したい操作は ON にしてください
...
- 各自の設定タブで「表示しない」を選んでいると出ません。位置は左下・右下から選べます
