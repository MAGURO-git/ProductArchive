...
2回触れると、当日スタッフを全員ぶん解除します（1回目で確認待ち、時間内に2回目で確定）。誤って付いた人の後始末や、イベント終了後に付与を残さないための片付けに使います。
...
SBSメニューのチャンネルカードは、12CH以下では3列、13〜16CHでは4列に並びます。既定の6チャンネルまでは1画面に収まり、それを超えるぶんは右端の ▲▼ ボタンで送ります（全体CHは下に固定で残ります）。
