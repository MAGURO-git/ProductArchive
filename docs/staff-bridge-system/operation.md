...
「Sceneで選ぶ」を押すとそのエリアが選択された状態で Scene ビューに映るので、緑の枠の点をドラッグして部屋の壁に合わせてください。
...
`BoxCollider` が決めるのは「どこまでを部屋とみなすか（誰が中に居るか）」で、上の2つの距離が決めるのは「中の人の声が外へどこまで届くか」です。役割が別なので、既定では片方を変えてももう片方は変わりません。
...
| モード | 動き |
|---|---|
| 手動 | 連動しません。箱も距離もそれぞれ自由に決めます |
| 箱に合わせる | Scene ビューで部屋に合わせた箱から距離を出します。距離の欄は編集できなくなり、ずれていると「箱に合わせる」ボタンが出ます |
| 距離に合わせる | 入れた「聞こえなくなる距離」から箱を作り直します。Scene ビューで箱を触る必要がありません |

どちらのモードでも **部屋の床の対角 ＝ 聞こえなくなる距離** で対応させ、「全音量で聞こえる距離」はその 2/3（既定 3m / 4.5m と同じ比）になります。値は 0.5m 刻みに丸められます。

- 対角に使うのは床（幅と奥行き）だけです。**箱の高さと位置は変わりません。**
- 「距離に合わせる」で作られる箱は正方形です。細長い部屋は、このモードで大きさの当たりを付けてから Scene ビューで整えるか、「箱に合わせる」を使ってください。
- 箱を Scene ビューでドラッグしている最中は距離を書き換えません。手を離してからボタンで反映します。
