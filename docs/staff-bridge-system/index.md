...
VRChat ワールド用のイベント運営支援ギミック一式です。テレポートメニュー・インカム・一斉メッセージ・進行タイマーなどを、スタッフの手元メニューにまとめて導入できます。
</div>
...
- 進行 — 共有タイマーと自分用タイマー／ノート（進行台本と個人メモ）／通知ログ／入退室ログ
...
イベントごとの作り込みは Unity 上の専用ウィンドウから行います。メニュー構成・テレポート先・定型メッセージ・配色テーマ・スタッフ名簿を Inspector を直接触らずに編集でき、設定の不備は診断・修復でまとめて直せます。
