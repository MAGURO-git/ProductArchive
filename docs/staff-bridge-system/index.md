...
**プレイヤーの声を操作する他のギミックとは同時に使えません。** 個室・ボイスゾーン・拡声・声の距離やゲインを変えるギミックは、インカムと同じ VRChat の音声設定を書き換えるため、どちらかが意図どおりに動かなくなります。同じワールドに入れる場合は、[インカムの一時停止](operation.md#incom-power)でどちらかを止めて使ってください。
...
**スタッフ限定の機能は、通常の VRChat クライアントの利用を前提としています。** Udon のイベントは技術的には誰からでも呼べるため、入退場のロック解除のような重大な操作は、送り先のギミック側でも呼び出し元を確認する作りにしてください。
